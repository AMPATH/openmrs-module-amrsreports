...
@Handler(supports = CohortRestrictedBirthdateDataDefinition.class, order = 50)
public class CohortRestrictedBirthdateDataEvaluator implements PersonDataEvaluator {
	...
	public EvaluatedPersonData evaluate(PersonDataDefinition definition, EvaluationContext context) throws EvaluationException {
		...
		if (context != null && context.getBaseCohort() != null && !context.getBaseCohort().isEmpty()) {
			...
			String hql = "select personId, birthdate, birthdateEstimated" +
					" from Person" +
					" where voided = false " +
					" and personId in (" +
			...
			for (Object o : queryResult) {
				...
				if (parts.length == 3) {
					...
					Date bDate = (Date) parts[1];
					Boolean bEst = (Boolean) parts[2];
					if (bDate != null)
						c.addData(pId, new Birthdate(bDate, bEst));
				}
			}
		}
